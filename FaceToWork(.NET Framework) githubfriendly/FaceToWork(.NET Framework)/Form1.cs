...
namespace FaceToWork
{
    public partial class Form1 : Form
    {
        ...
        internal Image<Bgr, Byte> videoframe2;

        ResizeBilinear filter_cubic = new ResizeBilinear(527, 302);

        bool oblicejdetekovan = false;
        byte[] jpegparams;
        ...
        private void Device_NewFrame2(object sender, NewFrameEventArgs eventArgs)
        {
            ...
            Bitmap video = filter_cubic.Apply((Bitmap)eventArgs.Frame.Clone());
            videoframe2 = new Image<Bgr, byte>(video);
            GrayFrame = new Image<Gray, byte>(video);
        ...
        {
            try //zatim nic nedela Jen deletuje group "lidi" 
            {
        ...
        private async void btnProcess_Click(object sender, EventArgs e) //posílá údaje do Face API
        {
            ...
            jpegparams = videoframe2.ToJpegData(60);// snímá uloží jpeg data do memory 
        ...
        public async void IdentifyIN(string _groupId)
        {
            try
            {
                if (device.IsRunning == false)// pokud kamera nema vstup - error
                ...
                foreach (var identifyResult in identifyResults)
                {
                    if (identifyResult.Candidates.Count != 0)
                    {
                        ...
                        idList.Items.Add($"{DateTime.Now.Date.ToString("dd.MM.yyyy")} {person.Name} přišel v {time}");
                        IN(time, person.Name, _groupId); //NOVEJ UPDATE
                    }
                    else
                    {
        ...
        public async void IdentifyOUT(string _groupId)
        {
            try
            {
                if (device2.IsRunning == false)// pokud kamera nema vstup - error
                {
                    MessageBox.Show("Chybí obrazový vstup z kamery", "Camera Input");
                    return;
                }
                ...
                foreach (var identifyResult in identifyResults)
                {
                    if (identifyResult.Candidates.Count != 0)
                    {
                        ...
                        Person person = await faceClient.PersonGroupPerson.GetAsync(_groupId, candidateId);
                        var time = DateTime.Now.ToString("HH:mm:ss");
                        idList.Items.Add($"{DateTime.Now.Date.ToString("dd.MM.yyyy")} {person.Name} odešel v {time}");
                        OUT(person.Name);//NOVEJ UPDATE
                    }
                    else
                    {
        ...
        private void IN(string dateIn, string personName, string groupId)
        {
            DataSet1TableAdapters.RecordTBLTableAdapter recordTBL = new DataSet1TableAdapters.RecordTBLTableAdapter();
            var date = DateTime.Now.ToString();
            recordTBL.AddRecord(date, personName, groupId, dateIn); //novej update
        }

        private void OUT(string PersonName)
        {
            DataSet1TableAdapters.RecordTBLTableAdapter recordTBL = new DataSet1TableAdapters.RecordTBLTableAdapter();
